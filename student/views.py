...
def lesson(request, lesson):
    ...
    if lesson[0] == "A":
        ...
        work_dict = dict(
            (
                (work.index, [work, WorkFile.objects.filter(work_id=work.id).order_by("-id")]) for work in Work.objects.filter(typing=0, lesson_id=lesson_id, user_id=request.user.id).select_related('user')
            )
        )
        # 限登入者
...
class WorkListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if len(work_pool)>0:
            ...
            work_end = work_pool[len(work_pool)-1].publication_date
        else:
