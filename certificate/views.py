...
def upload_pic(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            path_list = ["certification/", "cretification/1/", "certification/1/0/"]
            for p in path_list:
...
def make_image(lesson, unit, enroll_id, teacher_id):
    ...
    im = Image.open(settings.BASE_DIR / f"static/certificate/certificate{lesson}_{unit}.jpg") # create the image
...
def make(request):
    ...
    if classroom_id and enroll_id and action :
        try :
            enroll = Enroll.objects.get(id=enroll_id).select_related('classroom')
            lesson = enroll.classroom.lesson
            if action == 'certificate':
                ...
                try :
                    os.remove(settings.BASE_DIR / f"static/certificate/{unit}/{enroll_id}.jpg")
                except:
                    pass
            enroll.save()
        except ObjectDoesNotExist :
